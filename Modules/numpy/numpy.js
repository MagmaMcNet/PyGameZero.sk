! function(factory) { "function" == typeof define && define.amd ? define([], factory) : factory() }(function() {
    "use strict";
    var prop, M = Math,
        N = Number,
        def = Object.defineProperty,
        mathXtra = { sinh: function(x) { if (0 === x) return x; var exp = M.exp(x); return exp / 2 - .5 / exp }, cosh: function(x) { var exp = M.exp(x); return exp / 2 + .5 / exp }, tanh: function(x) { if (0 === x) return x; if (0 > x) { var exp = M.exp(2 * x); return (exp - 1) / (exp + 1) } var exp = M.exp(-2 * x); return (1 - exp) / (1 + exp) }, asinh: function(x) { return x === -(1 / 0) ? -(1 / 0) : M.log(x + M.sqrt(x * x + 1)) }, acosh: function(x) { return x >= 1 ? M.log(x + M.sqrt(x * x - 1)) : NaN }, atanh: function(x) { return x >= -1 && 1 >= x ? M.log((1 + x) / (1 - x)) / 2 : NaN }, expm1: function(x) { return 0 === x ? x : M.exp(x) - 1 }, log10: function(x) { return M.log(x) / M.LN10 }, log2: function(x) { return M.log(x) / M.LN2 }, log1p: function(x) { return 0 === x ? x : M.log(1 + x) }, sign: function(x) { return isNaN(x) ? NaN : 0 > x ? -1 : x > 0 ? 1 : +x }, cbrt: function(x) { return 0 === x ? x : 0 > x ? -M.pow(-x, 1 / 3) : M.pow(x, 1 / 3) }, hypot: function(value1, value2) { for (var i = 0, s = 0, args = arguments; i < args.length; i++) s += args[i] * args[i]; return M.sqrt(s) }, trunc: function(x) { return 0 === x ? x : 0 > x ? M.ceil(x) : M.floor(x) }, fround: "function" == typeof Float32Array ? function(arr) { return function(x) { return arr[0] = x, arr[0] } }(new Float32Array(1)) : function(x) { return x }, clz32: function(x) { if (x === -(1 / 0)) return 32; if (0 > x || (x |= 0) < 0) return 0; if (!x) return 32; for (var i = 31; x >>= 1;) i--; return i }, imul: function(x, y) { return (0 | x) * (0 | y) | 0 } },
        numXtra = { isNaN: function(x) { return "number" == typeof x && x !== x }, isFinite: function(x) { return "number" == typeof x && x === x && x !== 1 / 0 && x !== -(1 / 0) }, isInteger: function(x) { return "number" == typeof x && x !== 1 / 0 && x !== -(1 / 0) && M.floor(x) === x }, isSafeInteger: function(x) { return "number" == typeof x && x > -9007199254740992 && 9007199254740992 > x && M.floor(x) === x }, parseFloat: parseFloat, parseInt: parseInt },
        numConsts = { EPSILON: 2.220446049250313e-16, MAX_SAFE_INTEGER: 9007199254740991, MIN_SAFE_INTEGER: -9007199254740991 };
...
var $builtinmodule = function(name) {
    ...
    function _PyArray_ITER_NEXT1(it) { it.dataptr += it.strides[0], it.coordinates[0] += 1 }

    function _PyArray_ITER_NEXT2(it) { it.coordinates[1] < it.dims_m1[1] ? (it.coordinates[1] += 1, it.dataptr += it.strides[1]) : (it.coordinates[1] = 0, it.coordinates[0] += 1, it.dataptr += it.strides[0] - it.backstrides[1]) }

    function PyArray_ITER_NEXT(it) {
        if (it.index += 1, 0 == it.nd_m1) _PyArray_ITER_NEXT1(it);
        else if (1 == it.nd_m1) _PyArray_ITER_NEXT2(it);
        else { var __npy_i; for (__npy_i = it.nd_m1; __npy_i >= 0; __npy_i--) it.coordinates[__npy_i] < it.dims_m1[__npy_i] ? (it.coordinates[__npy_i] += 1, it.dataptr += it.strides[__npy_i]) : (it.coordinates[__npy_i] = 0, it.dataptr += it.backstrides[__npy_i]) }
    }

    function PyArray_ITER_RESET(it) { it.index = 0, it.dataptr = 0, it.coordinates = [0, it.nd_m1 + 1] }

    function PyArray_DESCR(arr) { return arr.v.dtype }
    ...
    function PyArray_ToScalar(data, arr) { return PyArray_Scalar(data, PyArray_DESCR(arr), arr) }

    function PyArray_Return(mp) { if (null == mp) return null; if (!PyArray_Check(mp)) return mp; if (0 == PyArray_NDIM(mp)) { var ret = PyArray_ToScalar(PyArray_DATA(mp), mp); return ret } return mp }
    ...
    function PyArray_SIZE(arr) { if (PyArray_Check(arr)) return prod(arr.v.shape); throw new Error("Internal API-Call Error occured in PyArray_DATA.", arr) }

    function PyArray_DATA(arr) { if (PyArray_Check(arr)) return arr.v.buffer; throw new Error("Internal API-Call Error occured in PyArray_DATA.", arr) }

    function PyArray_STRIDES(arr) { if (PyArray_Check(arr)) return arr.v.strides; throw new Error("Internal API-Call Error occured in PyArray_STRIDES.", arr) }

    function PyArray_STRIDE(arr, n) { if (PyArray_Check(arr)) { var strides = arr.v.strides; return strides[n] } throw new Error("Internal API-Call Error occured in PyArray_STRIDE.", arr) }

    function PyArray_NDIM(arr) { if (PyArray_Check(arr)) return arr.v.shape.length; throw new Error("Internal API-Call Error occured in PyArray_NDIM.", arr) }

    function PyArray_DIMS(arr) { if (PyArray_Check(arr)) return arr.v.shape; throw new Error("Internal API-Call Error occured in PyArray_DIMS.", arr) }

    function PyArray_DIM(arr, n) { if (PyArray_Check(arr)) return arr.v.shape[n]; throw new Error("Internal API-Call Error occured in PyArray_DIM.", arr) }

    function PyArray_NewShape(arr, shape, order) {
        if (PyArray_Check(arr)) {
            var py_shape = new Sk.builtin.tuple(shape.map(function(x) { return new Sk.builtin.int_(x) })),
    ...
    function PyArray_FLAGS(arr) { if (PyArray_Check(arr)) return arr.v.flags; throw new Error("Internal API-Call Error occured in PyArray_NewShape.", arr) }
    ...
    function OBJECT_vdot(ip1, is1, ip2, is2, op, n, ignore) {
        ...
        for (i = 0; n > i; i++, ip1_i += is1, ip2_i += is2) {
            if (null == ip1[ip1_i] || null == ip2[ip2_i]) tmp1 = Sk.builtin.bool.false$;
            else { if (tmp0 = Sk.misceval.callsim(ip1[ip1_i].conjugate, ip1[ip1_i]), null == tmp0) return; if (tmp1 = Sk.abstr.numberBinOp(tmp0, ip2[ip2_i], "Mult"), null == tmp1) return }
            if (0 === i) tmp = tmp1;
            else {
    ...
    function PyArray_DTypeFromObject(obj, maxdims) {
        function seqUnpacker(obj, mDims) { if (!Sk.builtin.checkSequence(obj)) return obj; var length = Sk.builtin.len(obj); if (Sk.builtin.asnum$(length) > 0) { var element = obj.mp$subscript(0); return mDims > 0 && Sk.builtin.checkSequence(element) ? seqUnpacker(element, mDims - 1) : element } }
        var dtype = null;
        ...
        else if (Sk.builtin.checkSequence(obj)) { if (element = seqUnpacker(obj, maxdims), PyArray_Check(element)) return PyArray_DESCR(obj) } else if (PyArray_Check(obj)) {
        ...
        try { dtype = Sk.builtin.type(element) } catch (e) {}
        return dtype
    }
    ...
    function PyArray_ObjectType(op, minimum_type) { var dtype; if (dtype = PyArray_DTypeFromObject(op, NPY_MAXDIMS), null == dtype) return NPY_DEFAULT_TYPE; var num = Internal_DType_To_Num(dtype); return num >= minimum_type ? num : minimum_type > num ? num >= 0 && 3 >= minimum_type ? minimum_type : NPY_NOTYPE : void 0 }

    function PyArray_Cast_Obj(obj, dtype) { return dtype instanceof Sk.builtin.none && Sk.builtin.checkNone(obj) ? obj : Sk.misceval.callsim(dtype, obj) }

    function PyArray_CheckExact(obj) { return PyArray_Check(obj) }

    function PyArray_CheckAnyScalarExact(obj) {}

    function PyArray_GetPriority(obj, default_) { var ret, priority = NPY_PRIORITY; return PyArray_CheckExact(obj) ? priority : PyArray_CheckAnyScalarExact(obj) ? NPY_SCALAR_PRIORITY : (ret = Sk.builtin.getattr(obj, new Sk.builtin.str("__array_priority__"), Sk.builtin.none.none$), Sk.builtin.checkNone(ret) ? default_ : (ret = Sk.builtin["float"](ret), priority = Sk.ffi.remapToJs(ret))) }
    ...
    function PyArray_NewFromDescr_int(subtype, descr, nd, dims, strides, data, flags, obj, zeroed) {
        ...
        for (i = 0; nd > i; i++) { var dim = dims[i]; if (null != dim) { if (0 > dim) throw new Sk.builtin.ValueError("negative dimensions are not allowed"); if (size = dim * size, size == Number.MAX_VALUE) throw new Sk.builtin.ValueError("array is too big.") } }
        fa = {}, fa.nd = nd, fa.dimensions = null, fa.data = null, null == data ? (fa.flags = NPY_ARRAY_DEFAULT, flags && flags && (fa.flags |= NPY_ARRAY_F_CONTIGUOUS, nd > 1 && (fa.flags &= ~NPY_ARRAY_C_CONTIGUOUS), flags = NPY_ARRAY_F_CONTIGUOUS)) : fa.flags = flags & ~NPY_ARRAY_UPDATEIFCOPY, fa.descr = descr, fa.base = null, fa.weakreflist = null, nd > 0 ? (fa.dimensions = [], fa.strides = [], dims.map(function(d) { fa.dimensions.push(d) }), null == strides ? sd = _array_fill_strides(fa.strides, dims, nd, sd, flags, fa.flags) : (strides.map(function(s) { fa.strides.push(s) }), sd *= size)) : (fa.dimensions = null, fa.strides = null, fa.flags |= NPY_ARRAY_F_CONTIGUOUS), null == data ? (0 == sd && (sd = 1), data = zeroed ? [] : [], fa.flags |= NPY_ARRAY_OWNDATA) : fa.flags &= ~NPY_ARRAY_OWNDATA, fa.data = data;
        var pyShape = new Sk.builtin.tuple((fa.dimensions || []).map(function(d) { return new Sk.builtin.int_(d) })),
    ...
    function PyArray_NewFromDescr(subtype, descr, nd, dims, strides, data, flags, obj) { return PyArray_NewFromDescr_int(subtype, descr, nd, dims, strides, data, flags, obj, 0) }

    function PyArray_New(subtype, nd, dims, type_num, strides, data, itemsize, flags, obj) { var descr, _new; return descr = PyArray_DescrFromType(type_num), null == descr ? null : _new = PyArray_NewFromDescr(subtype, descr, nd, dims, strides, data, flags, obj) }

    function new_array_for_sum(ap1, ap2, out, nd, dimensions, typenum) { var ret, subtype, prior1, prior2; if (Sk.builtin.type(ap2) != Sk.builtin.type(ap1) ? (prior = PyArray_GetPriority(ap2, 0), prior = PyArray_GetPriority(ap1, 0), subtype = prior2 > prior1 ? Sk.builtin.type(ap2) : Sk.builtin.type(ap1)) : (prior1 = prior2 = 0, subtype = Sk.builtin.type(ap1)), null != out) throw new Error('new_array_for_sum does not support "out" parameter'); return ret = PyArray_New(subtype, nd, dimensions, typenum, null, null, 0, 0, prior2 > prior1 ? ap2 : ap1) }
    ...
    function PyArray_FromAny(op, dtype, min_depth, max_depth, requirements, context) {
        ...
        var _shape = new Sk.builtin.tuple(state.shape.map(function(x) { return new Sk.builtin.int_(x) })),
            _buffer = new Sk.builtin.list(elements);
        return Sk.misceval.callsim(mod[CLASS_NDARRAY], _shape, dtype, _buffer)
    }

    function convert_shape_to_string(n, vals, ending) { var i, ret, tmp; for (i = 0; n > i && vals[i] < 0; i++); if (i === n) return Sk.abstr.numberBinOp(new Sk.builtin.str("()%s"), new Sk.builtin.str(ending), "Mod"); for (ret = Sk.abstr.numberBinOp(new Sk.builtin.str("(%i"), vals[i++], "Mod"); n > i; ++i) tmp = vals[i] < 0 ? new Sk.builtin.str(",newaxis") : Sk.abstr.numberBinOp(new Sk.builtin.str(",%i"), vals[i], "Mod"), ret = Sk.abstr.numberBinOp(ret, tmp, "Add"); return tmp = 1 == i ? Sk.abstr.numberBinOp(new Sk.builtin.str(",)%s"), new Sk.builtin.str(ending), "Mod") : Sk.abstr.numberBinOp(new Sk.builtin.str(")%s"), new Sk.builtin.str(ending), "Mod"), ret = Sk.abstr.numberBinOp(ret, tmp, "Add") }
    ...
    function PyTypeNum_ISFLEXIBLE(type) { return !1 }

    function PyTypeNum_ISEXTENDED(type) { return PyTypeNum_ISFLEXIBLE }
    ...
    function remapToJs_shallow(obj, shallow) { var _shallow = shallow || !0; if (obj instanceof Sk.builtin.list) { if (_shallow) return obj.v; for (var ret = [], i = 0; i < obj.v.length; ++i) ret.push(Sk.ffi.remapToJs(obj.v[i])); return ret } return obj instanceof Sk.builtin.float_ ? Sk.builtin.asnum$nofloat(obj) : Sk.ffi.remapToJs(obj) }
    ...
    function computeOffset(strides, index) { for (var offset = 0, k = 0, len = strides.length; len > k; k++) offset += strides[k] * index[k]; return offset }

    function prod(numbers) { var i, size = 1; for (i = 0; i < numbers.length; i++) size *= numbers[i]; return size }

    function tolistrecursive(buffer, shape, strides, startdim, dtype) { var i, n, stride, arr, item; if (startdim >= shape.length) return buffer[0]; for (n = shape[startdim], stride = strides[startdim], arr = [], i = 0; n > i; i++) item = tolistrecursive(buffer, shape, strides, startdim + 1, dtype), arr.push(item), buffer = buffer.slice(stride); return new Sk.builtin.list(arr) }

    function tolist(buffer, shape, strides, dtype) { var buffer_copy = buffer.slice(0); return tolistrecursive(buffer_copy, shape, strides, 0, dtype) }
    ...
    function callTrigonometricFunc(x, op) {
        ...
        if ((x instanceof Sk.builtin.list || x instanceof Sk.builtin.tuple) && (x = Sk.misceval.callsim(mod.array, x)), PyArray_Check(x)) {
            var _buffer = PyArray_DATA(x).map(function(value) { return num = Sk.builtin.asnum$(value), res = op.call(null, num), new Sk.builtin.float_(res) }),
                shape = new Sk.builtin.tuple(PyArray_DIMS(x).map(function(d) { return new Sk.builtin.int_(d) }));
    ...
    var CLASS_NDARRAY = "numpy.ndarray",
        ...
        numpy = function() { this.math = Math };
    numpy.prototype.arange = function(start, stop, step) { void 0 === step && (step = 1), start *= 1, stop *= 1, step *= 1; for (var res = [], i = start; stop > i; i += step) res.push(i); return res };
    var NPY_NOTYPE = null,
        NPY_DEFAULT_TYPE = 2,
        Internal_TypeTable = { complex: 3, complex_: 3, complex64: 3, "float": 2, float_: 2, float64: 2, "int": 1, int_: 1, int64: 1, bool: 0, bool_: 0 },
    ...
    var ndarray_f = function($gbl, $loc) {
        function makeNumericBinaryOpLhs(operation) {
            return function(self, other) {
                ...
                return shape = new Sk.builtin.tuple(PyArray_DIMS(self).map(function(x) { return new Sk.builtin.int_(x) })), buffer = new Sk.builtin.list(_buffer), Sk.misceval.callsim(mod[CLASS_NDARRAY], shape, PyArray_DESCR(self), buffer)
            }
        }
        ...
        function makeNumericBinaryOpRhs(operation) { return function(self, other) { for (var rhsBuffer = PyArray_DATA(self), _buffer = [], i = 0, len = rhsBuffer.length; len > i; i++) _buffer[i] = Sk.abstr.numberBinOp(other, rhsBuffer[i], operation); var shape = new Sk.builtin.tuple(PyArray_DIMS(self).map(function(x) { return new Sk.builtin.int_(x) })); return buffer = new Sk.builtin.list(_buffer), Sk.misceval.callsim(mod[CLASS_NDARRAY], shape, PyArray_DESCR(self), buffer) } }

        function makeUnaryOp(operation) {
            return function(self) {
                var _buffer = PyArray_DATA(self).map(function(value) { return Sk.abstr.numberUnaryOp(value, operation) }),
                    shape = new Sk.builtin.tuple(PyArray_DIMS(self).map(function(x) { return new Sk.builtin.int_(x) }));
        ...
        $loc.__init__ = new Sk.builtin.func(function(self, shape, dtype, buffer, offset, strides, order) {
            ...
            if (null != name && (Sk.builtin.checkString(name) || "string" == typeof name)) {
                ...
                switch (Sk.builtin.checkString(name) && (_name = Sk.ffi.remapToJs(name)), _name) {
                    ...
                    case "dtype":
                        return self.v.dtype;
                    case "shape":
                        return new Sk.builtin.tuple(PyArray_DIMS(self).map(function(x) { return new Sk.builtin.int_(x) }));
                    case "strides":
                        return new Sk.builtin.tuple(PyArray_STRIDES(self).map(function(x) { return new Sk.builtin.int_(x) }));
                    case "size":
            ...
            var r, f, ret;
            if (void 0 !== self.tp$getattr && (f = self.tp$getattr("__getattribute__")), void 0 !== f && (ret = Sk.misceval.callsimOrSuspend(f, new Sk.builtin.str(_name))), void 0 === r) throw new Sk.builtin.AttributeError("'" + Sk.abstr.typeName(self) + "' object has no attribute '" + _name + "'");
            return r
            ...
            var buffer = (Sk.ffi.remapToJs(self), PyArray_DATA(self).map(function(x) { return x })),
                shape = new Sk.builtin.tuple(PyArray_DIMS(self).map(function(x) { return new Sk.builtin.int_(x) }));
            ...
            if (Sk.builtin.pyCheckArgs("[]", arguments, 3, 3), Sk.builtin.checkInt(index)) {
                ...
                if (ndarrayJs.shape.length > 1) {
                    ...
                    for (i = _offset * _stride; _ubound > i; i++) ndarrayJs.buffer[i] = _value[_index++]
                } else {
        ...
        }), $loc.__len__ = new Sk.builtin.func(function(self) { var ndarrayJs = Sk.ffi.remapToJs(self); return new Sk.builtin.int_(ndarrayJs.shape[0]) }), $loc.__iter__ = new Sk.builtin.func(function(self) {
            var ndarrayJs = Sk.ffi.remapToJs(self),
                ret = { tp$iter: function() { return ret }, $obj: ndarrayJs, $index: 0, tp$iternext: function() { return ret.$index >= ret.$obj.buffer.length ? void 0 : ret.$obj.buffer[ret.$index++] } };
            return ret
            ...
            var _buffer = PyArray_DATA(self).map(function(value) { return Sk.builtin.pow(value, other) }),
                shape = new Sk.builtin.tuple(PyArray_DIMS(self).map(function(x) { return new Sk.builtin.int_(x) }));
            return buffer = new Sk.builtin.list(_buffer), Sk.misceval.callsim(mod[CLASS_NDARRAY], shape, PyArray_DESCR(self), buffer)
        }), $loc.dot = new Sk.builtin.func(function(self, other) { var ret; return ret = Sk.misceval.callsim(mod.dot, self, other) }), $loc.__abs__ = new Sk.builtin.func(function(self) {
            Sk.builtin.pyCheckArgs("__abs__", arguments, 1, 1);
            var _buffer = PyArray_DATA(self).map(function(value) { return Sk.builtin.abs(value) }),
                shape = new Sk.builtin.tuple(PyArray_DIMS(self).map(function(x) { return new Sk.builtin.int_(x) }));
            return buffer = new Sk.builtin.list(_buffer), Sk.misceval.callsim(mod[CLASS_NDARRAY], shape, PyArray_DESCR(self), buffer)
    ...
    var sin_f = function(x, out) { return Sk.builtin.pyCheckArgs("sin", arguments, 1, 2), callTrigonometricFunc(x, np.math ? np.math.sin : Math.sin) };
    sin_f.co_varnames = ["x", "out"], sin_f.$defaults = [0, new Sk.builtin.list([])], mod.sin = new Sk.builtin.func(sin_f);
    var sinh_f = function(x, out) { if (Sk.builtin.pyCheckArgs("sinh", arguments, 1, 2), !np.math) throw new Sk.builtin.OperationError("sinh requires math polyfill"); return callTrigonometricFunc(x, np.math.sinh) };
    sinh_f.co_varnames = ["x", "out"], sinh_f.$defaults = [0, new Sk.builtin.list([])], mod.sinh = new Sk.builtin.func(sinh_f);
    var arcsin_f = function(x, out) { return Sk.builtin.pyCheckArgs("arcsin", arguments, 1, 2), callTrigonometricFunc(x, np.math ? np.math.asin : Math.asin) };
    arcsin_f.co_varnames = ["x", "out"], arcsin_f.$defaults = [0, new Sk.builtin.list([])], mod.arcsin = new Sk.builtin.func(arcsin_f);
    var cos_f = function(x, out) { return Sk.builtin.pyCheckArgs("cos", arguments, 1, 2), callTrigonometricFunc(x, np.math ? np.math.cos : Math.cos) };
    cos_f.co_varnames = ["x", "out"], cos_f.$defaults = [0, new Sk.builtin.list([])], mod.cos = new Sk.builtin.func(cos_f);
    var cosh_f = function(x, out) { if (Sk.builtin.pyCheckArgs("cosh", arguments, 1, 2), !np.math) throw new Sk.builtin.OperationError("cosh requires math polyfill"); return callTrigonometricFunc(x, np.math.cosh) };
    cosh_f.co_varnames = ["x", "out"], cosh_f.$defaults = [0, new Sk.builtin.list([])], mod.cosh = new Sk.builtin.func(cosh_f);
    var arccos_f = function(x, out) { return Sk.builtin.pyCheckArgs("arccos", arguments, 1, 2), callTrigonometricFunc(x, np.math ? np.math.acos : Math.acos) };
    arccos_f.co_varnames = ["x", "out"], arccos_f.$defaults = [0, new Sk.builtin.list([])], mod.arccos = new Sk.builtin.func(arccos_f);
    var arctan_f = function(x, out) { return Sk.builtin.pyCheckArgs("arctan", arguments, 1, 2), callTrigonometricFunc(x, np.math ? np.math.atan : Math.atan) };
    arctan_f.co_varnames = ["x", "out"], arctan_f.$defaults = [0, new Sk.builtin.list([])], mod.arctan = new Sk.builtin.func(arctan_f);
    var tan_f = function(x, out) { return Sk.builtin.pyCheckArgs("tan", arguments, 1, 2), callTrigonometricFunc(x, np.math ? np.math.tan : Math.tan) };
    tan_f.co_varnames = ["x", "out"], tan_f.$defaults = [0, new Sk.builtin.list([])], mod.tan = new Sk.builtin.func(tan_f);
    var tanh_f = function(x, out) { if (Sk.builtin.pyCheckArgs("tanh", arguments, 1, 2), !np.math) throw new Sk.builtin.OperationError("tanh requires math polyfill"); return callTrigonometricFunc(x, np.math.tanh) };
    tanh_f.co_varnames = ["x", "out"], tanh_f.$defaults = [0, new Sk.builtin.list([])], mod.tanh = new Sk.builtin.func(tanh_f);
    var exp_f = function(x, out) { return Sk.builtin.pyCheckArgs("exp", arguments, 1, 2), callTrigonometricFunc(x, np.math ? np.math.exp : Math.exp) };
    exp_f.co_varnames = ["x", "out"], exp_f.$defaults = [0, new Sk.builtin.list([])], mod.exp = new Sk.builtin.func(exp_f);
    var sqrt_f = function(x, out) { return Sk.builtin.pyCheckArgs("sqrt", arguments, 1, 2), callTrigonometricFunc(x, np.math ? np.math.sqrt : Math.sqrt) };
    ...
    var linspace_f = function(start, stop, num, endpoint, retstep) {
        ...
        if (start_num = 1 * Sk.builtin.asnum$(start), stop_num = 1 * Sk.builtin.asnum$(stop), 0 >= num_num) samples = [];
            ...
            var samples_array;
            endpoint_bool ? 1 == num_num ? samples = [start_num] : (step = (stop_num - start_num) / (num_num - 1), samples_array = np.arange(0, num_num), samples = samples_array.map(function(v) { return v * step + start_num }), samples[samples.length - 1] = stop_num) : (step = (stop_num - start_num) / num_num, samples_array = np.arange(0, num_num), samples = samples_array.map(function(v) { return v * step + start_num }))
        }
    ...
    var asanyarray_f = function(a, dtype, order) { return Sk.misceval.callsim(mod.array, dtype, Sk.builtin.bool.false$, order) };
    ...
    var zeros_f = function(shape, dtype, order) { if (Sk.builtin.pyCheckArgs("zeros", arguments, 1, 3), !Sk.builtin.checkSequence(shape) && !Sk.builtin.checkInt(shape)) throw new Sk.builtin.TypeError('argument "shape" must int or sequence of ints'); if (dtype instanceof Sk.builtin.list) throw new Sk.builtin.TypeError("'" + Sk.abstr.typeName(dtype) + "' is not supported for dtype."); var _zero = new Sk.builtin.float_(0); return Sk.misceval.callsim(mod.full, shape, _zero, dtype, order) };
    ...
    var abs_f = function(x) { Sk.builtin.pyCheckArgs("abs", arguments, 1, 1); var ret; return ret = 1 == PyArray_Check(x) ? Sk.misceval.callsim(x.__abs__, x) : Sk.builtin.abs(x) };
    ...
    var mean_f = function(x, axis, dtype, out, keepdims) {
        ...
        if (1 == PyArray_Check(x)) {
            for (_buffer = PyArray_DATA(x), length = new Sk.builtin.int_(PyArray_SIZE(x)), i = 0; i < length.v; i++) sum = Sk.abstr.numberBinOp(sum, _buffer[i], "Add");
    ...
    var sum_f = function(x, axis, dtype, out, keepdims) {
        ...
        var _buffer, length, sum = new Sk.builtin.float_(0),
            i = 0;
        if (null != axis && !Sk.builtin.checkNone(axis)) throw new Sk.builtin.NotImplementedError("the 'axis' parameter is currently not supported");
        if (null != out && !Sk.builtin.checkNone(out)) throw new Sk.builtin.NotImplementedError("the 'out' parameter is currently not supported");
        if (null != keepdims && keepdims != Sk.builtin.bool.false$) throw new Sk.builtin.NotImplementedError("the 'keepdims' parameter is currently not supported");
        ...
        return null == dtype || Sk.builtin.checkNone(dtype) || (sum = Sk.misceval.callsim(dtype, sum)), sum
    };
    sum_f.co_varnames = ["a", "axis", "dtype", "out", "keepdims"], sum_f.$defaults = [null, Sk.builtin.none.none$, Sk.builtin.none.none$, Sk.builtin.none.none$, Sk.builtin.bool.false$], mod.sum = new Sk.builtin.func(sum_f);
    var prod_f = function(x, axis, dtype, out, keepdims) {
        Sk.builtin.pyCheckArgs("prod", arguments, 1, 5);
        var _buffer, length, prod = new Sk.builtin.float_(1),
            i = 0;
        if (null != axis && !Sk.builtin.checkNone(axis)) throw new Sk.builtin.NotImplementedError("the 'axis' parameter is currently not supported");
        if (null != out && !Sk.builtin.checkNone(out)) throw new Sk.builtin.NotImplementedError("the 'out' parameter is currently not supported");
        if (null != keepdims && keepdims != Sk.builtin.bool.false$) throw new Sk.builtin.NotImplementedError("the 'keepdims' parameter is currently not supported");
        if (1 == PyArray_Check(x))
            for (_buffer = PyArray_DATA(x), length = new Sk.builtin.int_(PyArray_SIZE(x)), i = 0; i < length.v; i++) prod = Sk.abstr.numberBinOp(prod, _buffer[i], "Mult");
        else prod = x;
        return null == dtype || Sk.builtin.checkNone(dtype) || (prod = Sk.misceval.callsim(dtype, prod)), prod
    };
    prod_f.co_varnames = ["a", "axis", "dtype", "out", "keepdims"], prod_f.$defaults = [null, Sk.builtin.none.none$, Sk.builtin.none.none$, Sk.builtin.none.none$, Sk.builtin.bool.false$], mod.prod = new Sk.builtin.func(prod_f);
    var ones_f = function(shape, dtype, order) { if (Sk.builtin.pyCheckArgs("ones", arguments, 1, 3), !Sk.builtin.checkSequence(shape) && !Sk.builtin.checkInt(shape)) throw new Sk.builtin.TypeError('argument "shape" must int or sequence of ints'); if (dtype instanceof Sk.builtin.list) throw new Sk.builtin.TypeError("'" + Sk.abstr.typeName(dtype) + "' is not supported for dtype."); var _one = new Sk.builtin.float_(1); return Sk.misceval.callsim(mod.full, shape, _one, dtype, order) };
    ...
    var dot_f = function(a, b, o) { Sk.builtin.pyCheckArgs("dot", arguments, 2, 3); var o, ret; if (Sk.builtin.checkNone(o) && (o = null), null != o && !PyArray_Check(o)) throw new Sk.builtin.TypeError("'out' must be an array"); return ret = MatrixProdcut2(a, b, o), PyArray_Return(ret) };
    ...
    var vdot_f = function(a, b) {
        ...
        if (op1 = PyArray_NewShape(ap1, newdims, "NPY_CORDER"), null != op1) {
            ...
            if (op2 = PyArray_NewShape(ap2, newdims, "NPY_CORDER"), null != op2) {
                ...
                var shape = new Sk.builtin.tuple([0].map(function(x) { return new Sk.builtin.int_(x) }));
    ...
    var less_f = function(x1, x2, out) { return Sk.builtin.pyCheckArgs("less", arguments, 2, 3, !1), compareLogical("Lt", x1, x2, out) };
    ...
    var less_equal_f = function(x1, x2, out) { return Sk.builtin.pyCheckArgs("less_equal", arguments, 2, 3, !1), compareLogical("LtE", x1, x2, out) };
    ...
    var greater_f = function(x1, x2, out) { return Sk.builtin.pyCheckArgs("greater", arguments, 2, 3, !1), compareLogical("Gt", x1, x2, out) };
    ...
    var greater_equal_f = function(x1, x2, out) { return Sk.builtin.pyCheckArgs("greater_equal", arguments, 2, 3, !1), compareLogical("GtE", x1, x2, out) };
    ...
    var equal_f = function(x1, x2, out) { return Sk.builtin.pyCheckArgs("equal", arguments, 2, 3, !1), compareLogical("Eq", x1, x2, out) };
    ...
    var not_equal_f = function(x1, x2, out) { return Sk.builtin.pyCheckArgs("not_equal", arguments, 2, 3, !1), compareLogical("NotEq", x1, x2, out) };
