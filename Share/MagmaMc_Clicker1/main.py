...
Smoke = []
for i in range(1,7):
    actor = Actor("Smoke_0"+str(i), pos=(-200,-200))
    actor._orig_surf = pygame.transform.scale(actor._orig_surf, (40, 40))
    actor._update_pos()
    Smoke.append(actor)
...
box_none4 = Actor("box_none", center=(170, 430))
box_tick4 = Actor("box_tick", center=(170, 430))
...
box_none4._orig_surf = pygame.transform.scale(box_none._orig_surf, (40, 40))
box_tick4._orig_surf = pygame.transform.scale(box_tick._orig_surf, (40, 40))
...
box_none4._update_pos()
box_tick4._update_pos()
...
class PlayerData:
    def SAVE():
        filez.fwrite(str(Cookies.get("UserID"))+'.json', json.dumps(save, sort_keys=True, indent=4), "c")
    def GET():
        return str(filez.fread(str(Cookies.get("UserID"))+'.json'))
...
reloading = 0
def leaderboard_get():
    global leaderboard, reloading
    reloading = 20
    leaderboard = {}
    Save_files = (filez.scan('/'))
    
    for file in Save_files:
        content = json.loads(str(filez.fread(str(file))))
        leaderboard.update( ({content["Name"]: int(content["Coins"]) }) )
    
    leaderboard = Counter(leaderboard).most_common(5)
    reloading = 1
leaderboard_get()
...
clock.schedule_interval(leaderboard_get, 2)

def draw():
    global player_animate, animation, a, reloading
    #470 true
    ...
    if mode == "Menu":
        ...
        for user in leaderboard:
            if reloading != 0:
                reloading -= 1
                screen.draw.text("Reloading", pos=(980, (13)*35), size=30, color="black", bold="true")
                break
            screen.draw.text("Leaderboard", center=(1050, (11)*35), fontsize=30, gcolor="orange", color="red", bold="true")
            screen.draw.text(str(user[0]), pos=(950, (12+user_leaderboard)*35), size=20, color="black", bold="true")
            screen.draw.text(str(user[1]), pos=(1100, (12+user_leaderboard)*35), size=20, color="black", bold="true")
            user_leaderboard += 1
        ...
        screen.draw.text("FPS: ", pos=(15,410), fontsize=30, color="black", bold="true")
    ...
    if Show_FPS:
        FPS_Modules.update
        if int(FPS_Modules.display) < 15:
            screen.draw.text(FPS_Modules.display, pos=(10, 10), fontsize=20, bold="true", color="red")
        elif int(FPS_Modules.display) < 25:
            screen.draw.text(FPS_Modules.display, pos=(10, 10), fontsize=20, bold="true", color=(255, 80, 0))
        else:
            screen.draw.text(FPS_Modules.display, pos=(10, 10), fontsize=20, bold="true", color=(0, 200, 0))
...
def on_mouse_down(button, pos):
    ...
    global sound, music, animation, Show_FPS
    if mode == "Skins":
        a = 0
        for i in Red_buttons:
            a += 1
